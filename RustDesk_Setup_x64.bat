...
echo ==========================================================================================
echo L O A D I N G   ^|   Universal RustDesk Stable Installer Downloader
echo ==========================================================================================
echo.
...
rem Default fallback to standard x64 architecture link (Hardcoded version 1.4.5)
set "download_url=https://github.com/rustdesk/rustdesk/releases/download/1.4.5/rustdesk-1.4.5-x86_64.exe"
set "filename=rustdesk-1.4.5-x86_64.exe"
set "arch_type=x64 (64-bit)"

rem Check if the OS environment is 32-bit to swap to the x86 build dynamically
set "is_64=0"
if "%PROCESSOR_ARCHITECTURE%"=="AMD64" set "is_64=1"
if "%PROCESSOR_ARCHITEW6432%"=="AMD64" set "is_64=1"

if "%is_64%"=="0" (
    set "download_url=https://github.com/rustdesk/rustdesk/releases/download/1.4.5/rustdesk-1.4.5-x86.exe"
    set "filename=rustdesk-1.4.5-x86.exe"
...
echo   * It automatically fetches the specified stable release from official GitHub servers.
...
set "new_dir=C:\Windows\Temp\rustdesk_dynamic_session_%RANDOM%_%RANDOM%_%RANDOM%"
mkdir "%new_dir%" 2>nul

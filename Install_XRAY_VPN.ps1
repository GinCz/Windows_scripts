...
Write-Host '       XRAY_VPN AUTOMATED INSTALLER FOR WINDOWS 7 / 10 / 11        ' -ForegroundColor Cyan
Write-Host '====================================================================' -ForegroundColor Cyan
Write-Host ''
...
# 1. Download xray.exe directly from HTTP
if (-not (Test-Path "$dir\xray.exe")) {
    ...
    $exeName = if ($is64) { 'xray64.exe' } else { 'xray32.exe' }
    $exeUrl = 'http://prodvig-saita.ru/vpn/' + $exeName

    Write-Host ('[*] Downloading ' + $exeName + ' from server...') -ForegroundColor Cyan
    $wc = New-Object System.Net.WebClient
    $wc.Headers.Add('User-Agent', 'Mozilla/5.0')
    $wc.DownloadFile($exeUrl, "$dir\xray.exe")
    Write-Host ('[+] xray.exe downloaded: ' + (Get-Item "$dir\xray.exe").Length + ' bytes') -ForegroundColor Green
} else {
    Write-Host '[+] xray.exe already present' -ForegroundColor Green
}
